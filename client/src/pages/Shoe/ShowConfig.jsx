...
import { useGLTF, ContactShadows, Environment, OrbitControls,Html } from "@react-three/drei"
...
export default function ShowConfig() {
  ...
  const downloadCanvasToImage = (gl, scene, camera) => {
    ...
    document.body.removeChild(link);
      axios.post('http://localhost:8080/api/v1/products/'+auth.id, {
        ...
        stock: true,
        rating: 0,
        description: "this shoe is a good buy for everyday wear.",
        trending: true,
        size: 38,
        img: "/"+filename
      }).then(response=>{
  ...
  return (
    <div className="w-[100vw] h-[80vh]">
      <Canvas shadows camera={{ position: [0, 0, 4], fov: 45 }}>
        ...
        <Shoe downloadCanvasToImage={downloadCanvasToImage} canvasToImage={canvasToImage} setCanvasToImage={setCanvasToImage}/>
      ...
      <div
          ...
          >
            <button className="bg-primary-black  hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={()=>setCanvasToImage(true)}>Save</button>
          </div>
    </div>
  )
}
...
function Shoe(props) {
  ...
  useEffect(() => {
    ...
    if(props.canvasToImage){
      props.downloadCanvasToImage(gl, scene, camera)
      setTimeout(props.setCanvasToImage(false)
      , 1000);
    }
  }, [props.canvasToImage])
  return (
...
function Picker() {
  ...
  return (
    <>
    <div style={{ display: snap.current ? "block" : "none" }} className="absolute top-[200px] left-[240px]">
      <HexColorPicker className="picker" color={snap.items[snap.current]} onChange={(color) => (state.items[snap.current] = color)} />
      <h1 className="absolute top-[180px] left-[70px]">{snap.current}</h1>
    </div>
